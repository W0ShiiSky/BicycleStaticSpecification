...
$(function () {
    ...
    async function loadModel() {
        const modelPath = 'C:/Users/ewanh/java2/BicycleStaticSpecification/runs/train/yolov5s_results/weights/best.onnx';
    ...
    async function detectObjects() {
        ...
        const preprocessed = expanded.toFloat().div(tf.scalar(255));

        // Run inference
        const predictions = await model.executeAsync(preprocessed);
        
        // Process predictions and render on canvas
        renderPredictions(predictions);

        // Cleanup
        tf.dispose([img, resized, expanded, preprocessed]);
    }

    // Function to render predictions on canvas and dashboard
    function renderPredictions(predictions) {
        ...
        predictions.forEach(prediction => {
            ...
            const labelDiv = $('<div>').text(label);
            dashboard.append(labelDiv);
        });
    }

    // Initialize
    Promise.all([loadModel(), startVideo()]).then(() => {
        // Add detection loop
        setInterval(detectObjects, 1000); // Adjust interval as needed
    }).catch(error => {
